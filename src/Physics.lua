...
function CalculatePath(x, y, dir, startclr)
    ...
    while true do -- just break if you need to get out of the loop
        ...
        elseif ccell.id == "wall" then
            break
        elseif ccell.id == "mirror" then -- crazy bouncing shit
            local rdir = cdir
        ...
        elseif ccell.id == "amplifier" then -- seperate because we want the laser to survive afterwards
            local side = (ccell.rot - cdir) % 4
        ...
        elseif ccell.id == "tunnel" then -- ok the tunnel is coming back (code time)
            local side = (ccell.rot - cdir) % 4
            if side == 0 then
                local ccx,ccy = cx,cy
                local depth = 1
                while depth > 0 do
                    ...
                    if OnGrid(ccx,ccy) then
                        local cccell = GetNextCell(ccx,ccy)
                        if cccell.id == "tunnel" then
