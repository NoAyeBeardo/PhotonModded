...
function SaveSelectionScreenshot()
    if not SelectionHUD.active then
        print("SaveSelectionScreenshot: SelectionHUD is not active") -- Debug log
        return
    end

    print("SaveSelectionScreenshot: Saving screenshot...") -- Debug log
    local screenshotFolder = "Screenshots"
    if not love.filesystem.createDirectory(screenshotFolder) then
        print("SaveSelectionScreenshot: Failed to create Screenshots folder") -- Debug log
        return
    end

    local date = os.date("*t")
    local filename = string.format(
        "%s/Screenshot_%02d-%02d-%02d [%02d-%02d-%02d].png",
        screenshotFolder,
    ...
    local scaleFactor = 4 -- Increase this value for higher resolution
    local canvasWidth = SelectionHUD.width * CellSize * scaleFactor
    local canvasHeight = SelectionHUD.height * CellSize * scaleFactor
    ...
    for x = 0, SelectionHUD.width - 1 do
        for y = 0, SelectionHUD.height - 1 do
            local screenX = x * CellSize
            local screenY = y * CellSize
            love.graphics.draw(Tex["empty"], screenX, screenY, 0, 1, 1)
        end
    end
    love.graphics.setColor(1, 1, 1, 1) -- Reset color after rendering the grid

    -- Render the lasers
    local offset = CellSize / 2
    for x = SelectionHUD.x1, SelectionHUD.x2 do
        for y = SelectionHUD.y1, SelectionHUD.y2 do
            if OnGrid(x, y) then
                local laserCells = GetLaserCell(x, y)
                if laserCells then
                    local sx = (x - SelectionHUD.x1) * CellSize + offset
                    local sy = (y - SelectionHUD.y1) * CellSize + offset
                    for _, laser in ipairs(laserCells) do
                        love.graphics.setColor(1, 1, 1, 1) -- Reset color for each laser
                        DrawCell(laser, sx, sy, 1)
                    end
                end
            end
        end
    end
    love.graphics.setColor(1, 1, 1, 1) -- Reset color after rendering lasers
    ...
    for x = SelectionHUD.x1, SelectionHUD.x2 do
        for y = SelectionHUD.y1, SelectionHUD.y2 do
            if OnGrid(x, y) then
                ...
                local sx = (x - SelectionHUD.x1) * CellSize + offset
                local sy = (y - SelectionHUD.y1) * CellSize + offset
                if cell and cell.id ~= "empty" then
                    DrawCell(cell, sx, sy, 1)
                end
                love.graphics.setColor(1, 1, 1, 1) -- Reset color after rendering each cell
            end
        end
    end
    love.graphics.setColor(1, 1, 1, 1) -- Reset color after rendering cells

    love.graphics.setBlendMode("alpha") -- Reset blend mode to default
    love.graphics.pop()
    love.graphics.setCanvas()

    local imageData = canvas:newImageData()
    if not imageData then
        print("SaveSelectionScreenshot: Failed to create ImageData") -- Debug log
        return
    end

    if not imageData:encode("png", filename) then
        print("SaveSelectionScreenshot: Failed to save screenshot to file") -- Debug log
        return
    end

    print("SaveSelectionScreenshot: Screenshot saved as " .. filename) -- Debug log
end